...
class OpenAlexTool(BaseTool):
    ...
    def __init__(self, download_pdfs: bool = True, save_dir: str = "./pdfs", default_max_results: int = 50, **kwargs):
    ...
    def _run(self, query: str, max_results: int = -1, from_publication_date: Optional[str] = None) -> List[dict]:
        try:
            ...
            if self.download_pdfs:
                ...
                for paper in papers:
                    if paper['pdf_url']:
                        # Prefer arXiv ID, then DOI (sanitized), then fall back to sanitized title
                        if paper['arxiv_id']:
