...
class ResearchFlow(Flow[ScientificResearchState]):
    ...
    def __init__(self):
        super().__init__()
        self.llm = LLM(model="gemini/gemini-2.0-flash", temperature=0.3, api_key=os.getenv("GEMINI_API_KEY"))
    ...
    def _get_output_path(self, filename):
        return str(self._get_base_dir() / "output" / filename)
    ...
    def collect_publications(self):
        ...
        collect_task = Task(
            ...
            output_file=self._get_output_path("collected_publications.json")
        )
        ...
        publications_text = self.state.collected_publications or ""
        if "unable to provide" in publications_text.lower() or "no publications" in publications_text.lower() or len(publications_text.strip()) < 100:
            error_msg = f"""NO PUBLICATIONS FOUND

            Topic: {self.state.topic}
            Timeframe: Last {self.state.timeframe} years (from {datetime.now().year - self.state.timeframe} onwards)

            The publication collector was unable to find relevant scientific publications for this topic within the specified timeframe.

            Recommendations:
            - Broaden the search terms
            - Increase the timeframe (e.g., 10-15 years for established topics)
            - Try a different research question"""
            
            with open(self._get_output_path("final_related_work.md"), "w", encoding="utf-8") as f:
                f.write(error_msg)
            
            self.state.final_chapter = error_msg
            self.state.current_step = "completed"
            
            return "No publications found - flow stopped"
    ...
    def filter_publications(self):  
        if self.state.current_step == "completed":
            return "Flow stopped - no publications found"
        ...
        if "unable to provide" in publications_text.lower() or "no publications" in publications_text.lower() or len(publications_text.strip()) < 100:
            error_msg = f"""
            RESEARCH FLOW STOPPED: NO PUBLICATIONS FOUND

            Topic: {self.state.topic}
            Timeframe: Last {self.state.timeframe} years (from {datetime.now().year - self.state.timeframe} onwards)
            
            The publication collector was unable to find relevant scientific 
            publications for this topic within the specified timeframe.
            
            Possible reasons:
            - The topic may be too specific or use uncommon terminology
            - The timeframe ({self.state.timeframe} years) may be too restrictive
            - The topic may be an established field with older foundational research
            
            Recommendations:
            - Broaden the search terms or topic description
            - Increase the timeframe (e.g., 10-15 years for established topics)
            - Try a different or more specific research question
            """
            print(error_msg)
            
            with open(self._get_output_path("02_filtered_publications.json"), "w", encoding="utf-8") as f:
                f.write(error_msg)
            with open(self._get_output_path("final_related_work.md"), "w", encoding="utf-8") as f:
                f.write(error_msg)

            self.state.filtered_publications = error_msg
            self.state.final_chapter = error_msg
            self.state.current_step = "completed"
        ...
        filter_task = Task(
            ...
            output_file=self._get_output_path("02_filtered_publications.json")
        )
        ...
        try:
            ...
            filtered_text = self.state.filtered_publications
            # danke copilot
            if filtered_text.strip().startswith('```'):
                match = re.search(r'```(?:json)?\s*\n(.*?)\n```', filtered_text, re.DOTALL)
                if match:
                    filtered_text = match.group(1)
                    print("  Removed markdown wrapper from filtered publications")
            
            filtered_pubs = json.loads(filtered_text)
            if not isinstance(filtered_pubs, list) or len(filtered_pubs) == 0:
                raise ValueError("No publications in filtered list")
        except (json.JSONDecodeError, ValueError) as e:
            error_msg = f"Leider wurden keine relevanten Paper zu {self.state.topic} gefunden."
    ...
    def _cleanup_and_upsert_filtered_publications(self):
        try:
            ...
            collected_pubs = json.loads(clean_json(self.state.collected_publications))
            filtered_pubs = json.loads(clean_json(self.state.filtered_publications))
            
            filtered_ids = set()

            for pub in filtered_pubs:
                doi = (pub.get('doi') or '').strip()
                arxiv_id = (pub.get('arxiv_id') or '').strip()
    ...
    def process_publications(self):
        if self.state.current_step == "completed":
            return "Flow stopped - no relevant publications found"
        ...
        process_task = Task(
            ...
            output_file=self._get_output_path("03_processed_content.md")
        )
    ...
    def write_chapter(self):
        ...
        if is_revision:
            ...
            output_file = self._get_output_path(f"05_chapter_revision_iteration_{self.state.iteration_count}.md")
        else:
            ...
            output_file = self._get_output_path("chapter_initial_draft.md")
    ...
    def evaluate_chapter(self):
        ...
        try:
            ...
            #danke an copilot für die regex hilfe
            ...
            try:
                with open(self._get_output_path("evaluation.json"), "a", encoding="utf-8") as f:
    ...
    def quality_decision_router(self):
        if self.state.evaluation.score >= self.state.target_score:
            ...
            try:
                with open(self._get_output_path("iteration_count.json"), "a", encoding="utf-8") as lf:
    ...
    def finalize_chapter(self):
        ...
        self._save_chapter(self._get_output_path("final_related_work.md"), self.state.final_chapter)
...
def kickoff():
    print("Starting Research Flow with Evaluator-Optimizer Loop")
    flow = ResearchFlow()
    result = flow.kickoff()
