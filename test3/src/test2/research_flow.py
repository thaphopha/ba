...
class ScientificResearchState(BaseModel):
    ...
    collected_publications: List[PublicationData] = Field(default_factory=list)
    filtered_publications: List[PublicationData] = Field(default_factory=list)
...
class ResearchFlow(Flow[ScientificResearchState]):
    ...
    def collect_publications(self):
        ...
        collection_crew = Crew(
            ...
            verbose=True,
            output_log_file="logs/publication_collection.json"
        )
    ...
    def filter_publications(self):
        ...
        self.state.filtered_publications = self.state.collected_publications[:15]
        self.state.current_step = "processing"

        print(f"Filtered to {len(self.state.filtered_publications)} high-quality publications")
